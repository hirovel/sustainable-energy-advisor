...
- **从一个小实验开始。** 不同时改变整套生活，只验证一个最有信息量的变量。
...
> 你今天还有四小时工作，但“完成眼前任务”和“确认有没有更好的工具”正在争用同一份注意力。落后感又不断把注意力拉回工具比较，每次切换之后还要重新进入任务。身体层面的信息还不够，所以暂时不在睡眠或体能上做判断。
>
> 我会先验证切换是不是主要的精力泄漏。接下来三个工作日，开始核心任务前写下本次使用的工具和完成标准，在任务结束前不再比较模型。想刷新时只做一个记号，不立刻切换。每天结束看三件事：实际切换次数、最长连续工作时段，以及任务有没有实质推进。
>
> 睡眠、运动和长期计划先保持不变。三个工作日可以先看看这个变量有没有明显影响，也避免同时改太多东西，让结果失去解释力。
